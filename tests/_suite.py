...
class Version(object):
    ...
    def benchmark(self, runner):  # pragma: no cover
        for test in self.tests():
            runner.bench_func(
                name=test.fully_qualified_name, func=test.validate_ignoring_errors
            )
    ...
    def tests(self):
        return (
            ...
            for test in self._tests_in(subject=child.basename()[:-5], path=child)
        )
    ...
    def optional_tests_of(self, name):
        return self._tests_in(
            subject=name, path=self._path.descendant(["optional", name + ".json"])
        )
...
class _Test(object):
    ...
    def fully_qualified_name(self):  # pragma: no cover
        return " > ".join(
            [self.version.name, self.subject, self.case_description, self.description]
        )
