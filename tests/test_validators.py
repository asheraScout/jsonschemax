...
class TestCreateAndExtend(SynchronousTestCase):
    def setUp(self):
        self.addCleanup(
            self.assertEqual, validators.meta_schemas, dict(validators.meta_schemas)
        )
    ...
    def test_attrs(self):
        self.assertEqual(
            ...
            (self.validators, self.meta_schema, self.type_checker),
        )
    ...
    def test_if_a_version_is_provided_it_is_registered(self):
        Validator = validators.create(
            meta_schema={u"$id": "something"}, version="my version"
        )
    ...
    def test_extend(self):
        ...
        Extended = validators.extend(self.Validator, validators={u"new": new})
        self.assertEqual(
            (
                Extended.VALIDATORS,
...
class TestLegacyTypeCheckCreation(SynchronousTestCase):
    ...
    def test_default_types_used_if_no_type_checker_given(self):
        Validator = validators.create(
            meta_schema=self.meta_schema, validators=self.validators
        )
    ...
    def test_default_types_update_type_checker(self):
        ...
        Extended = validators.extend(
            Validator, type_checker=Validator.TYPE_CHECKER.remove(u"array")
        )
    ...
    def test_types_redefines_the_validators_type_checker(self):
        ...
        validator = validators.Draft7Validator(schema, types={"string": (str, int)})
...
class TestLegacyTypeCheckingDeprecation(SynchronousTestCase):
    ...
    def test_providing_explicit_type_checker_does_not_warn(self):
        Validator = validators.create(
            meta_schema={}, validators={}, type_checker=TypeChecker()
        )
...
class TestValidationErrorMessages(TestCase):
    ...
    def test_dependencies_single_element(self):
        ...
        message = self.message_for(
            instance={"bar": 2}, schema=schema, cls=validators.Draft3Validator
        )
    ...
    def test_dependencies_list_draft7(self):
        ...
        schema = {u"dependencies": {depend: [on]}}
        message = self.message_for(
            instance={"bar": 2}, schema=schema, cls=validators.Draft7Validator
        )
        self.assertEqual(message, "%r is a dependency of %r" % (on, depend))

    def test_additionalItems_single_failure(self):
        message = self.message_for(
            instance=[2], schema={u"items": [], u"additionalItems": False}
        )
        self.assertIn("(2 was unexpected)", message)

    def test_additionalItems_multiple_failures(self):
        message = self.message_for(
            instance=[1, 2, 3], schema={u"items": [], u"additionalItems": False}
        )
    ...
    def test_additionalProperties_multiple_failures(self):
        schema = {u"additionalProperties": False}
        message = self.message_for(
            instance=dict.fromkeys(["foo", "bar"]), schema=schema
        )
    ...
    def test_const(self):
        schema = {u"const": 12}
        message = self.message_for(
            instance={"foo": "bar"}, schema=schema, cls=validators.Draft6Validator
        )
    ...
    def test_contains(self):
        ...
        message = self.message_for(
            instance=[2, {}, []], schema=schema, cls=validators.Draft6Validator
        )
        self.assertIn("None of [2, {}, []] are valid under the given schema", message)
    ...
    def test_invalid_format_default_message(self):
        ...
        message = self.message_for(
            instance="bla", schema=schema, format_checker=checker
        )
    ...
    def test_additionalProperties_false_patternProperties(self):
        ...
        message = self.message_for(
            instance={u"zebra": 123}, schema=schema, cls=validators.Draft4Validator
        )
        self.assertEqual(
            message,
    ...
    def test_False_schema(self):
        message = self.message_for(
            instance="something", schema=False, cls=validators.Draft7Validator
        )
...
class TestValidationErrorDetails(TestCase):
    ...
    def test_propertyNames(self):
        ...
        self.assertEqual(
            error.message, "%r is not allowed for %r" % ({"const": "foo"}, "foo")
        )
...
class ValidatorTestMixin(MetaSchemaTestsMixin, object):
    ...
    def test_it_can_validate_with_decimals(self):
        ...
        Validator = validators.extend(
            ...
            type_checker=self.Validator.TYPE_CHECKER.redefine(
                "number",
                lambda checker, thing: isinstance(thing, (int, float, Decimal))
        ...
        self.assertEqual(
            [error.instance for error in validator.iter_errors(invalid)], invalid
        )
    ...
    def test_non_string_custom_type(self):
        ...
        Crazy = validators.extend(
            self.Validator,
            type_checker=self.Validator.TYPE_CHECKER.redefine(
                non_string_type, lambda checker, thing: isinstance(thing, int)
            ),
        )
    ...
    def test_it_properly_formats_tuples_in_errors(self):
        ...
        TupleValidator = validators.extend(
            self.Validator,
            type_checker=self.Validator.TYPE_CHECKER.redefine(
                "array", lambda checker, thing: isinstance(thing, tuple)
            ),
        )
        with self.assertRaises(exceptions.ValidationError) as e:
...
for Validator, checker in (
    ...
    for format in checker.checkers:
        ...
        name = "test_{}_{}_ignores_non_strings".format(Validator.__name__, format)
        test.__name__ = name
...
class TestValidatorFor(SynchronousTestCase):
    ...
    def test_custom_validator_draft6(self):
        Validator = validators.create(
            meta_schema={"$id": "meta schema $id"}, version="13"
        )
        schema = {"$schema": "meta schema $id"}
        self.assertIs(validators.validator_for(schema), Validator)
...
class TestValidate(SynchronousTestCase):
    ...
    def test_validation_error_message(self):
        ...
        self.assertRegexpMatches(
            str(e.exception), "(?s)Failed validating u?'.*' in schema.*On instance"
        )
    ...
    def test_schema_error_message(self):
        ...
        self.assertRegexpMatches(
            str(e.exception), "(?s)Failed validating u?'.*' in metaschema.*On schema"
        )
...
class TestRefResolver(SynchronousTestCase):
    ...
    def setUp(self):
        ...
        self.resolver = validators.RefResolver(self.base_uri, self.referrer, self.store)
    ...
    def test_it_resolves_local_refs_with_id(self):
        ...
        resolver = validators.RefResolver.from_schema(
            schema, id_of=lambda schema: schema.get(u"id", u"")
        )
